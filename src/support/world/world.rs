use super::{
    ...
    Transform, WorldPhysics, WorldTexture,
};
...
pub struct World {
    ...
    pub hdr_textures: Vec<WorldTexture>,
...
impl World {
    ...
    pub fn load_hdr(&mut self, path: impl AsRef<Path>) -> Result<()> {
        self.hdr_textures.push(WorldTexture::from_hdr(path)?);
        Ok(())
    }
...
impl SdfFont {
    pub fn new(font_path: impl AsRef<Path>, texture_path: impl AsRef<Path>) -> Result<Self> {
        ...
        let texture = WorldTexture::from_file(texture_path)?;
