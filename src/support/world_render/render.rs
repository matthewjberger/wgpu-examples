use super::texture::from_world_texture;
use crate::{
    uniform::{
        DynamicUniform, DynamicUniformBinding, Geometry, TextureBinding, Uniform, UniformBinding,
    },
    AlphaMode, Material, MeshRender, RigidBody, Texture, Transform, World,
};
...
impl WorldRender {
    ...
    pub fn load(&mut self, device: &wgpu::Device, queue: &Queue, world: &World) -> Result<()> {
        let textures = world
            .textures
            .iter()
            .map(|world_texture| from_world_texture(device, queue, world_texture, "World Texture"))
